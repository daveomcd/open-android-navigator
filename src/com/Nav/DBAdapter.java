 ...
 * Note: My only change from the original code at this point is the addition 
 * of the insertLocationEntry function which is a modified entry function
 * made for inserting GPS locations into the database along with descriptions.
 */
...
public class DBAdapter{
	 ...
	 * This function should only be called by the appendData function of DatabaseQueryLocations.java
	 * @param contentValues ContentValues containing the new database information
	 * @return Returns the number of the row added
	 */
	public long insertLocationEntry(ContentValues contentValues)
	{
		String timeStamp = new Timestamp(Calendar.getInstance().getTimeInMillis()).toString();
		contentValues.put(KEY_TIMESTAMP, timeStamp);
		return db.insert(DATABASE_TABLE, null, contentValues);
